...
from utils.file_utils import load_pkl, save_pkl, add_colorbar
from matplotlib.colors import Normalize #jd
from matplotlib.cm import ScalarMappable #jd
...
class WholeSlideImage(object):
    def __init__(self, path):
        ...
        self.name = ".".join(path.split("/")[-1].split('.')[:-1]) #diff
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0),
        ...
        only_mask = np.zeros(img.shape) #Add new
        ...
        only_mask = Image.fromarray(np.uint8(only_mask)) #Add new
    ...
    def isInHoles(holes, pt, patch_size):
        for hole in holes:
            #pt_tuple = tuple(pt)  #Add new:Convert numpy array to tuple:
            #print("Point to test:", pt)
            #print("Type of point:", type(pt))
            if cv2.pointPolygonTest(hole, (pt[0]+patch_size/2, pt[1]+patch_size/2), False) > 0: 
            #if cv2.pointPolygonTest(hole, (pt_tuple[0]+patch_size/2, pt_tuple[1]+patch_size/2), False) > 0: #c把上面的改成了这样
                return 1
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        fp_chunk_size = math.ceil(n_contours * 0.05) #向上取整
        init = True
        for idx, cont in enumerate(self.contours_tissue):
    ...
    def process_contours_images(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        name_str = str(self.name)
        save_path_images = os.path.join(save_path, str(patch_level), name_str)
        print("Creating patches for: ", self.name, "...",)
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            asset_dict, attr_dict = self.process_contour(cont, self.holes_tissue[idx], patch_level, save_path, patch_size, step_size, **kwargs)
            if len(asset_dict) > 0:
                patch_coords = asset_dict['coords']
                for patch_idx in range(0, len(patch_coords)):
                    x = patch_coords[patch_idx][0]
                    y = patch_coords[patch_idx][1]
                    x_loc = int(x /(2**patch_level))
                    y_loc = int(y /(2**patch_level))
                    patch = self.wsi.read_region((x, y), patch_level, (patch_size, patch_size))
                    patch = patch.convert("RGB")
                    patch_filename = f"{name_str}_{self.level_dim[patch_level][0]}_{self.level_dim[patch_level][1]}_{patch_level}_{x_loc}_{y_loc}.jpg"
                    patch_filepath = os.path.join(save_path_images, patch_filename)
                    patch.save(patch_filepath)
        return self.hdf5_file

    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
                        contour_fn='four_pt', mag='40', use_padding=True, top_left=None, bot_right=None): #Add new: mag='40'
        start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (0, 0, self.level_dim[patch_level][0], self.level_dim[patch_level][1])
    ...
    def process_coord_candidate(coord, contour_holes, ref_patch_size, cont_check_fn):
        if WholeSlideImage.isInContours(cont_check_fn, coord, contour_holes, ref_patch_size):
            ...
            return None
    def add_colorbar_to_image(self, image, colormap='coolwarm'):
        # 创建一个与图像大小相同的画布
        ...
        # 创建 ScalarMappable 对象以便使用 colormap
        norm = Normalize(vmin=0, vmax=255)  # 假设图像的像素值范围是 [0, 255]
        sm = ScalarMappable(norm=norm, cmap=colormap)

        # 添加 colorbar
        plt.colorbar(sm, ax=ax)

        # 将图像转换为 ScalarMappable 映射的值范围
        sm.set_array(image)

        # 将 matplotlib 图像转换为 PIL.Image
        fig.canvas.draw()
        colorbar_image_array = np.array(fig.canvas.renderer.buffer_rgba())

        # 将 colorbar 图像和原始图像合并
        merged_image_array = np.concatenate((image, colorbar_image_array), axis=1)

        # 将合并后的图像转换为 PIL.Image
    ...
    def visHeatmap(self, scores, coords, vis_level=-1,
                   ...
                   blur=True, overlap=0.0,
                   #segment=True, use_holes=True, #mg
        ...
        downsample = self.level_downsamples[vis_level]
        #scale = [1/downsample[0], 1/downsample[1]]  # Scaling from 0 to desired level #mg
        scale = [2/downsample[0], 2/downsample[1]] #c level3
        ...
        if blur:
            ...
            overlay = cv2.GaussianBlur(overlay, (31, 31), 5)
        ...
        if isinstance(cmap, str):
            ...
            # cmap = adjust_colormap(plt.get_cmap(cmap + '_r'), gamma=0.7) 
            # cmap = truncate_colormap(plt.get_cmap(cmap + '_r'), 0.1, 0.9)
            # cmap = listed_to_segmented(plt.get_cmap(cmap + '_r'))
        for idx in range(len(coords)):
        ...
        if blur:
            # 应用高斯模糊以平滑边缘#addnew
            # img = cv2.GaussianBlur(img, (151, 151), 20)  # ksize为11x11, sigma为10 #刚刚
        ...
        if alpha < 1.0:
            ...
            # 自己实现的方式
            canvas = np.array(self.wsi.read_region((0,0), vis_level, self.wsi.level_dimensions[vis_level]).convert("RGB")) #Add new c
            img = cv2.addWeighted(img, alpha, canvas, 1 - alpha, 0, canvas) #c

        img = Image.fromarray(img) #c
        w, h = img.size #c
        ...
        #添加颜色条 #addnew
        img = add_colorbar(img, cmap, norm=plt.Normalize(vmin=np.min(scores), vmax=np.max(scores)))
        return img
...
#     """ Truncate colormap to only use a subset of the original color range. """
#     new_cmap = mcolors.LinearSegmentedColormap.from_list(
#         'truncated_' + cmap.name, cmap(np.linspace(min_val, max_val, n)))
#     return new_cmap

# def adjust_colormap(cmap, gamma=1.0):
#     """ Adjust the contrast of a colormap using a gamma correction. """
#     if isinstance(cmap, mcolors.ListedColormap):
#         # For ListedColormap, adjust gamma by applying directly to the color list
#         colors = cmap(np.linspace(0, 1, cmap.N))
#         colors = np.power(colors, gamma)
#         return mcolors.ListedColormap(colors, name='adjusted_' + cmap.name)

#     elif isinstance(cmap, mcolors.LinearSegmentedColormap):
#         # For LinearSegmentedColormap, adjust using _segmentdata
#         cdict = cmap._segmentdata
#         for key in ('red', 'green', 'blue'):
#             cdict[key] = [(x, y**gamma, z**gamma) for x, y, z in cdict[key]]
#         return mcolors.LinearSegmentedColormap('adjusted_' + cmap.name, cdict)

#     else:
#         raise TypeError(f"Unsupported colormap type: {type(cmap)}")

# def listed_to_segmented(cmap):
#     """ Convert a ListedColormap to LinearSegmentedColormap. """
#     colors = cmap(np.linspace(0, 1, cmap.N))
#     cdict = {'red': [], 'green': [], 'blue': []}
#     for i, color in enumerate(colors):
#         r, g, b, _ = color
#         cdict['red'].append((i / (cmap.N - 1), r, r))
#         cdict['green'].append((i / (cmap.N - 1), g, g))
#         cdict['blue'].append((i / (cmap.N - 1), b, b))
    
#     return mcolors.LinearSegmentedColormap('converted_' + cmap.name, cdict)