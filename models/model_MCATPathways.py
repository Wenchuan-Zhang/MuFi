...
class MCATPathways(nn.Module):
    def __init__(self, fusion='concat', omic_sizes=[100, 200, 300, 400, 500, 600], n_classes=4,
        ...
        self.size_dict_WSI = {"small": [384, 256, 256], "big": [1024, 512, 384]}
    ...
    def forward(self, **kwargs):
        ...
        # h_path_bag = self.wsi_net(x_path).unsqueeze(1) ### path embeddings are fed through a FC layer #1 4096 384 > 1 1 4096 256
        h_path_bag = self.wsi_net(x_path).permute(1, 0, 2) ### path embeddings are fed through a FC layer #1 4096 384 > 4096 1 256
        ...
        # Coattn
        h_path_coattn, A_coattn = self.coattn(h_omic_bag, h_path_bag, h_path_bag) #241 1 256, 1 1 4096 256, 1 1 4096 256 > 241 1 256, 1 1 241 4096
        ...
        # ### Path2: Coattn
        # h_path_bag2 = self.wsi_net2(x_path2).permute(1, 0, 2) #1 n 192 > 1 n 256
        # h_path_coattn2, A_coattn2 = self.coattn(h_omic_bag, h_path_bag2, h_path_bag2)  #1 n 256, 241 1 256 > 1 1 241 n
        ...
        # ### Path3: Coattn
        # h_path_bag3 = self.wsi_net3(x_path3).permute(1, 0, 2) #1 2 768 > 1 2 256
        # h_path_coattn3, A_coattn3 = self.coattn(h_omic_bag, h_path_bag3, h_path_bag3)  #1 n 256, 241 1 256 > 1 1 241 n
        ...
        if self.fusion == 'bilinear':
            ...
            # h = self.mm(torch.cat([h_path, h_path2, h_path3, h_omic], axis=0))
...
def multi_head_attention_forward(
    ...
    """
    tens_ops = (query, key, value, in_proj_weight, in_proj_bias, bias_k, bias_v, out_proj_weight, out_proj_bias) #214 1 256, 4096 1 256, 4096 1 256, 768 256, 768, none, none, 256 256, 256
    ...
    attn_output_weights = torch.bmm(q, k.transpose(1, 2)) #1 241 256, 1 4096 256 > 1 241 4096
    ...
    attn_output = torch.bmm(attn_output_weights, v) #1 241 4096, 1 4096 256 > 1 241 256
