...
#############################
class MCATPathwaysMotCat(nn.Module):
    def __init__(self, fusion='concat', omic_sizes=[100, 200, 300, 400, 500, 600], n_classes=4,
        ...
        super(MCATPathwaysMotCat, self).__init__()
        self.num_pathways = len(omic_sizes) 
        self.fusion = fusion
        self.omic_sizes = omic_sizes
        self.n_classes = n_classes
        # self.size_dict_WSI = {"small": [1024, 256, 256], "big": [1024, 512, 384]}
        self.size_dict_WSI = {"small": [1024, 256, 256], "big": [1024, 512, 384]} #HIPT:384; CCL: 2048; UNI: 1024
        ...
        self.wsi_net2 = nn.Sequential(nn.Linear(192, size[1]), nn.ReLU(), nn.Dropout(0.25))
        self.wsi_net3 = nn.Sequential(nn.Linear(768, size[1]), nn.ReLU(), nn.Dropout(0.25))
    ...
    def forward(self, **kwargs):
        x_path = kwargs['x_path1']
        # x_path2 = kwargs['x_path2'] #1 n 192
        # x_path3 = kwargs['x_path3']
        # 将 x_path3 从 (1, 1, 768) 变成 (1, 2, 768)
        # x_path3 = torch.cat([x_path3, x_path3], dim=1)
        ...
        h_path_bag = self.wsi_net(x_path) #1 4096 384 > 1 4096 256
        ...
        ### Coattn, OT
        A_coattn, _ = self.coattn(h_path_bag, h_omic_bag)  #1 4096 256, 241 1 256 > 1 1 241 4096
        h_path_coattn = torch.mm(A_coattn.squeeze(), h_path_bag.squeeze()).unsqueeze(1) #1 1 241 4096, 1 4096 256 > 241 1 256

        ### Path
        h_path_trans = self.path_transformer(h_path_coattn) #241 1 256
        A_path, h_path = self.path_attention_head(h_path_trans.squeeze(1)) #241 1 256 > 241 1, 241 256
        A_path = torch.transpose(A_path, 1, 0) 
        h_path = torch.mm(F.softmax(A_path, dim=1) , h_path) #1 241, 241 256 > 1 256
        h_path = self.path_rho(h_path).squeeze() #1 256 > 256
        
        ### Omic
        h_omic_trans = self.omic_transformer(h_omic_bag) #241 1 256
        A_omic, h_omic = self.omic_attention_head(h_omic_trans.squeeze(1)) #241 1 256 > 241 1, 241 256
        A_omic = torch.transpose(A_omic, 1, 0)
        h_omic = torch.mm(F.softmax(A_omic, dim=1) , h_omic) #1 241, 241 256 > 1 256
        h_omic = self.omic_rho(h_omic).squeeze() #1 256 > 256
        
        ### Path2: Coattn, OT
        # h_path_bag2 = self.wsi_net2(x_path2) #1 n 192 > 1 n 256
        # A_coattn2, _ = self.coattn(h_path_bag2, h_omic_bag)  #1 n 256, 241 1 256 > 1 1 241 n
        # h_path_coattn2 = torch.mm(A_coattn2.squeeze(), h_path_bag2.squeeze()).unsqueeze(1) #1 1 241 n, 1 n 256 > 241 1 256

        ### Path2
        # h_path_trans2 = self.path_transformer(h_path_coattn2) #241 1 256
        # A_path2, h_path2 = self.path_attention_head(h_path_trans2.squeeze(1)) #241 1 256 > 241 1, 241 256
        # A_path2 = torch.transpose(A_path2, 1, 0) 
        # h_path2 = torch.mm(F.softmax(A_path2, dim=1) , h_path2) #1 241, 241 256 > 1 256
        # h_path2 = self.path_rho(h_path2).squeeze() #1 256 > 256

        # ### Path3: Coattn, OT
        # h_path_bag3 = self.wsi_net3(x_path3) #1 2 768 > 1 2 256
        # A_coattn3, _ = self.coattn(h_path_bag3, h_omic_bag)  #1 2 256, 241 1 256 > 1 1 241 2
        # h_path_coattn3 = torch.mm(A_coattn3.squeeze(), h_path_bag3.squeeze()).unsqueeze(1) #1 1 241 2, 1 2 256 > 241 1 256

        # ### Path3
        # h_path_trans3 = self.path_transformer(h_path_coattn3) #241 1 256
        # A_path3, h_path3 = self.path_attention_head(h_path_trans3.squeeze(1)) #241 1 256 > 241 1, 241 256
        # A_path3 = torch.transpose(A_path3, 1, 0) 
        # h_path3 = torch.mm(F.softmax(A_path3, dim=1) , h_path3) #1 241, 241 256 > 1 256
        # h_path3 = self.path_rho(h_path3).squeeze() #1 256 > 256
        
        if self.fusion == 'bilinear':
            h = self.mm(h_path.unsqueeze(dim=0), h_omic.unsqueeze(dim=0)).squeeze()
        elif self.fusion == 'concat':
            h = self.mm(torch.cat([h_path, h_omic], axis=0))
            # h = self.mm(torch.cat([h_path, h_path2, h_path3, h_omic], axis=0)) #256 256 256 256 > 256
