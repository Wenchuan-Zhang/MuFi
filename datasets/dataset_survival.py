...
# ALL_MODALITIES = ['rna_clean_demo.csv']  
ALL_MODALITIES = ['radiomics_features_591_SJ.csv']  #radiomics_features_202 / radiomics_features_591_SJ
class SurvivalDatasetFactory:
    ...
    def __init__(self,
        ...
        #---> self
        self.study = study #tcga_brca_demo
        self.label_file = label_file 
        self.omics_dir = omics_dir 
        self.seed = seed #1
        self.print_info = print_info #True
        self.train_ids, self.val_ids  = (None, None)
        self.data_dir = None
        self.label_col = label_col #response_lable
        self.n_bins = n_bins #2
        self.num_patches = num_patches #4096
        self.is_mcat = is_mcat #False
        self.is_survpath = is_survpath #True
        self.type_of_path = type_of_pathway #combine
    ...
    def _setup_mcat(self):
        ...
        for col in self.signatures.columns:
            ...
            omic = sorted(_series_intersection(omic, self.all_modalities["radiomics"].columns)) #radiomics, rna
    ...
    def _get_patient_data(self):
        ...
        patient_data = {'case_id': patients_df["case_id"].values, 'response_label': patients_df['response_label'].values}
    ...
    def _get_patient_dict(self):
        ...
        self.patient_dict = patient_dict #TCGA-3C-AAAU': array(['TCGA-3C-AAAU-01Z-00-DX1.F6E9A5DF-D8FB-45CF-B4BD-C6B76294C291.svs
    ...
    def _cls_ids_prep(self):
        ...
        for i in range(self.num_classes):
            ...
            self.patient_cls_ids[i] = np.where(self.patient_data['response_label'] == i)[0] 

        # Find the index of slides for different labels
        self.slide_cls_ids = [[] for i in range(self.num_classes)]
        for i in range(self.num_classes):
            # self.slide_cls_ids[i] = np.where(self.label_data['label'] == i)[0]
            self.slide_cls_ids[i] = np.where(self.label_data['response_label'] == i)[0]
    ...
    def _get_split_from_df(self, args, all_splits, split_key: str='train', fold = None, scaler=None, valid_cols=None):
        ...
        for key in args.dataset_factory.all_modalities.keys():
            ...
            df_metadata_slide.to_csv(f'/home/zwc/project/MuFi-main/df_metadata_slide_{split_key}_fold0.csv')
        ...
        split_dataset = SurvivalDataset(
            ...
            data_dir_cls256=args.data_root_dir_cls256, 
            data_dir_cls4096=args.data_root_dir_cls4096, 
            data_dir_clsoneslide=args.data_root_dir_clsoneslide,
...
class SurvivalDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.data_dir_cls256 = data_dir_cls256
        self.data_dir_cls4096 = data_dir_cls4096
        self.data_dir_clsoneslide = data_dir_clsoneslide
    ...
    def slide_cls_id_prep(self):
        ...
        for i in range(self.num_classes):
            self.slide_cls_ids[i] = np.where(self.metadata['response_label'] == i)[0]
        count_label_0 = len(self.slide_cls_ids[0]) 
        count_label_1 = len(self.slide_cls_ids[1])
        # Printing the counts
        print(f"Count of response_label 0: {count_label_0}")
        print(f"Count of response_label 1: {count_label_1}")
    ...
    def __getitem__(self, idx):
        ...
        if self.modality in ['omics', 'snn', 'mlp_per_path']:
            ...
            df_small = self.omics_data_dict["radiomics"][self.omics_data_dict["radiomics"]["temp_index"] == case_id]
            ...
            patch_features, patch_features1, patch_features2, mask = self._load_wsi_embs_from_path(self.data_dir_cls256, self.data_dir_cls4096, self.data_dir_clsoneslide, slide_ids)

            # return (torch.zeros((1,1)), omics_tensor, label, event_time, c, clinical_data)
            return (patch_features, omics_tensor, label, clinical_data, mask, patch_features1, patch_features2, case_id, idx)
        
        #@TODO what is the difference between tmil_abmil and transmil_wsi
        elif self.modality in ["mlp_per_path_wsi", "abmil_wsi", "abmil_wsi_pathways", "deepmisl_wsi", "deepmisl_wsi_pathways", "mlp_wsi", "transmil_wsi", "transmil_wsi_pathways","Frmil_wsi", "Frmil_wsi_pathways"]:
            ...
            return (patch_features, omics_tensor, label, clinical_data, mask, patch_features1, patch_features2, case_id, idx)
        elif self.modality in ["coattn", "coattn_motcat","coattn_cmta"]:
            ...
            for i in range(self.num_pathways):
                omic_list.append(torch.tensor(self.omics_data_dict["radiomics"][self.omic_names[i]].iloc[idx])) #radiomics, rna
            
            return (patch_features, omic_list, label, clinical_data, mask, patch_features1, patch_features2, case_id, idx)
            
        elif self.modality == "survpath":
            # patch_features, mask = self._load_wsi_embs_from_path(self.data_dir_cls256, self.data_dir_cls4096, self.data_dir_clsoneslide, slide_ids)
            patch_features, patch_features1, patch_features2, mask,coords = self._load_wsi_embs_from_path(self.data_dir_cls256, self.data_dir_cls4096, self.data_dir_clsoneslide, slide_ids)
            omic_list = []
            for i in range(self.num_pathways):
                omic_list.append(torch.tensor(self.omics_data_dict["radiomics"][self.omic_names[i]].iloc[idx])) #radiomics, rna
            
            # return (patch_features, omic_list, label, event_time, c, clinical_data, mask)
            return (patch_features, omic_list, label, clinical_data, mask, patch_features1, patch_features2, case_id, idx,coords)
    ...
    def get_data_to_return(self, idx):
        ...
        # count_label_0 = len(self.metadata[self.metadata['response_label'] == 0])
        # count_label_1 = len(self.metadata[self.metadata['response_label'] == 1])
        # Printing the counts
        # print(f"Count of response_label 0: {count_label_0}")
        # print(f"Count of response_label 1: {count_label_1}")
        label = torch.Tensor([self.metadata['response_label'][idx]])
        # print(f"label: {label}")
    ...
    def _load_wsi_embs_from_path(self, data_dir_cls256, data_dir_cls4096, data_dir_clsoneslide, slide_ids):
        """
        Load all the patch embeddings from a list a slide IDs. 

        Args:
            - self 
            - data_dir_cls256 : String 
            - data_dir_cls4096 : String
            - data_dir_clsoneslide : String
            - slide_ids : List
        ...
        """
        def load_features(data_dir, slide_ids, feature_key):
            features_list = []
            coords_list = []
            for slide_id in slide_ids:
                h5_path = os.path.join(data_dir, '{}.h5'.format(slide_id)) #.rstrip('.svs')
                with h5py.File(h5_path, 'r') as hf:
                    features = hf[feature_key][:]
                    coords = hf["coords"][:]
                features_list.append(torch.tensor(features))
                coords_list.append(torch.tensor(coords))
            return torch.cat(features_list, dim=0), torch.cat(coords_list, dim=0)

        def _load_features(data_dir, slide_ids, feature_key):
            features_list = []
            coords_list = []
            for slide_id in slide_ids:
                h5_path = os.path.join(data_dir, '{}.h5'.format(slide_id)) #.rstrip('.svs')
        ...
        def sample_features(features, max_patches):
            n_samples = min(features.shape[0], max_patches)
            idx = np.sort(np.random.choice(features.shape[0], n_samples, replace=False))
            sampled_features = features[idx, :]

            if n_samples < max_patches:
                zeros = torch.zeros([max_patches - n_samples, features.shape[1]])
                sampled_features = torch.cat([sampled_features, zeros], dim=0)
                mask = torch.cat([torch.zeros([n_samples]), torch.ones([max_patches - n_samples])])
            else:
                mask = torch.zeros([max_patches])

            return sampled_features, mask

        patch_features,coords = load_features(data_dir_cls256, slide_ids, 'features') #features_cls256, features
        patch_features1,_ = load_features(data_dir_cls4096, slide_ids, 'features_cls4k')
        patch_features2 = _load_features(data_dir_clsoneslide, slide_ids, 'last_layer_embed')

        if self.sample:
            max_patches = self.num_patches
            patch_features, mask = sample_features(patch_features, max_patches)
            # patch_features1, mask1 = sample_features(patch_features1, max_patches)
        else:
            mask = torch.ones([1])
            # mask1 = torch.ones([1])
        # for slide_id in slide_ids:
            # h5_path = os.path.join(data_dir_cls256, '{}.h5'.format(slide_id.rstrip('.svs')))
            # with h5py.File(h5_path, 'r') as hf:
                # features = hf['features'][:]
                # coords = hf['coords'][:]

            # features_tensor = torch.tensor(features)
            # patch_features.append(features_tensor)
        # patch_features = torch.cat(patch_features, dim=0)
        ...
        # for slide_id in slide_ids:
        #     h5_path = os.path.join(data_dir_cls4096, '{}.h5'.format(slide_id.rstrip('.svs')))
        #     with h5py.File(h5_path, 'r') as hf:
        #         features = hf['features'][:]
        #         coords = hf['coords'][:]

        #     features_tensor = torch.tensor(features)
        #     patch_features1.append(features_tensor)
        # patch_features1 = torch.cat(patch_features1, dim=0)
        
        # if self.sample:
        #     max_patches = self.num_patches

        #     n_samples = min(patch_features1.shape[0], max_patches)
        #     idx = np.sort(np.random.choice(patch_features1.shape[0], n_samples, replace=False))
        #     patch_features1 = patch_features1[idx, :]
        
        #     # make a mask 
        #     if n_samples == max_patches:
        #         # sampled the max num patches, so keep all of them
        #         mask1 = torch.zeros([max_patches])
        #     else:
        #         # sampled fewer than max, so zero pad and add mask
        #         original = patch_features1.shape[0]
        #         how_many_to_add = max_patches - original
        #         zeros = torch.zeros([how_many_to_add, patch_features1.shape[1]])
        #         patch_features1 = torch.concat([patch_features1, zeros], dim=0)
        #         mask1 = torch.concat([torch.zeros([original]), torch.ones([how_many_to_add])])
        
        # else:
        #     mask1 = torch.ones([1])
        
        # for slide_id in slide_ids:
        #     h5_path = os.path.join(data_dir_clsoneslide, '{}.h5'.format(slide_id.rstrip('.svs')))
        #     with h5py.File(h5_path, 'r') as hf:
        #         features = hf['features'][:]
        #         # coords = hf['coords'][:]

        #     features_tensor = torch.tensor(features)
        #     patch_features2.append(features_tensor)
        # patch_features2 = torch.cat(patch_features2, dim=0)
        
        # # if self.sample:
        #     # max_patches = self.num_patches

        #     # n_samples = min(patch_features2.shape[0], max_patches)
        #     # idx = np.sort(np.random.choice(patch_features2.shape[0], n_samples, replace=False))
        #     # patch_features2 = patch_features2[idx, :]
        ...
        return patch_features, patch_features1, patch_features2, mask,coords
    ...
    def get_clinical_data(self, case_id):
        ...
        try:
            subtype = self.clinical_data.loc[case_id, "subtypes"]
        except:
            subtype = "N/A"
        
        clinical_data = (age, subtype)
    ...
    def getlabel(self, idx):
        ...
        label = self.metadata['response_label'][idx]
        return label
